  ...
  private fun BleTelemetryDto.toDomain(device: BleDevice?): TelemetrySample {
    ...
    val rawDetectorId =
      listOf(detectorId, deviceId, device?.id?.value, device?.macAddress)
        ...
        ?.trim() ?: error("Detector identifier missing in telemetry payload")
    ...
    val radiationMetrics =
      RadiationMetrics(
        ...
        equivalentDoseMicrosieverts =
          radiationDto?.equivalentDoseMicrosieverts ?: equivalentDoseUsv,
        backgroundDoseMicrosievertsPerHour =
          radiationDto?.backgroundDoseMicrosievertsPerHour ?: backgroundDoseUsvh,
      )
    ...
    val environmentSnapshot =
      EnvironmentSnapshot(
        boardTemperatureCelsius =
          environmentDto?.boardTemperatureCelsius ?: boardTemperatureCelsius,
        sensorTemperatureCelsius =
          environmentDto?.sensorTemperatureCelsius ?: sensorTemperatureCelsius,
