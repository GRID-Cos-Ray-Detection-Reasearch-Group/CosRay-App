...
internal object FirmwarePacketMapper {
  ...
  private fun parseMuonPacket(macAddress: String, packetBytes: ByteArray): ParsedFirmwarePacket? {
    val packet =
      runCatching { Protocol.MuonDataPkg.fromRawData(packetBytes) }.getOrNull() ?: return null
  ...
  private fun parseTimelinePacket(
    ...
    val packet =
      runCatching { Protocol.TimeLinePkg.fromRawData(packetBytes) }.getOrNull() ?: return null
  ...
  private fun muonPacketSamples(
    ...
    return validEvents.mapIndexed { index, event ->
      ...
      TelemetrySample(
        ...
        acquisition = AcquisitionMetrics(particleCount = event.energy.toInt() and 0xFFFF),
  ...
  private fun timelinePacketSamples(
    ...
    return validEvents.mapIndexed { index, event ->
      ...
      TelemetrySample(
        ...
        environment =
          EnvironmentSnapshot(
            sipmTemperatureCelsius = (event.siPMTmp.toInt() and 0xFFFF).toDouble(),
            mcuTemperatureCelsius = (event.mcUTmp.toInt() and 0xFF).toDouble(),
          ),
        power = PowerSnapshot(),
        location =
          LocationSnapshot(
  ...
  private fun hasHeader(packetBytes: ByteArray, header: ByteArray): Boolean =
    packetBytes[0] == header[0] && packetBytes[1] == header[1] && packetBytes[2] == header[2]
